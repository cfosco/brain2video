    ...
    def __init__(self, path='../data/HCP/npz', roi='VC', patch_size=16, transform=identity, aug_times=2, 
        ...
        if include_hcp:
            for c, sub in enumerate(os.listdir(path)):
                if os.path.isfile(os.path.join(path,sub, 'REST1_LR','HCP_visual_voxel.npz')) == False:
                    continue 
...
class hcp_all_dataset(Dataset):
    def __init__(self, dataset_path='../data/HCP/npz', roi_list=None, patch_size=16, transform=identity, aug_times=2, 
                num_sub_limit=None, include_kam=False, include_hcp=True):
        super(hcp_all_dataset, self).__init__()
        data = []
        images = []
        ...
        if include_hcp:
            ...
            for c, sub in enumerate(os.listdir(dataset_path)):
                if os.path.isfile(os.path.join(dataset_path, sub, 'HCP_visual_voxel_task-rest.pkl')) == False:
                    continue 
                if num_sub_limit is not None and c > num_sub_limit:
                    break
                pkl = pickle.load(open(os.path.join(dataset_path, sub, 'HCP_visual_voxel_task-rest.pkl'), 'rb'))
                # npz = dict(np.load(os.path.join(path,sub, 'REST1_LR', 'HCP_visual_voxel.npz')))
                voxel_list = [pkl[r] for r in roi]
                
                voxels = np.concatenate(voxel_list, axis=1) # 1200, num_voxels
                voxels = process_voxel_ts(voxels, patch_size) # num_samples, num_voxels_padded
...
class bmd_dataset(Dataset):
    def __init__(self, dataset_path='../data/bmd/', roi_list=['V1', 'V2', 'V3', 'V4'], patch_size=16, transform=identity, subjects='sub-01', aug_times=2, 
                num_sub_limit=None, split='train'):
        super(bmd_dataset, self).__init__()
        data = []
        images = []
        ...
        for c, sub in enumerate(os.listdir(dataset_path)):
            if sub not in subjects:
                continue
            if os.path.isfile(os.path.join(dataset_path, sub, 'BMD_visual_voxel_task-{}_betas.pkl'.format(split))) == False:
                raise Exception('No file found')
            pkl = pickle.load(open(os.path.join(dataset_path, sub, 'BMD_visual_voxel_task-{}_betas.pkl'.format(split)), 'rb'))
            # npz = dict(np.load(os.path.join(path,sub, 'REST1_LR', 'HCP_visual_voxel.npz')))
            voxel_list = [pkl[r] for r in roi]
            ...
            voxels = process_voxel_ts(voxels, patch_size, t=1) # num_samples, num_voxels_padded
