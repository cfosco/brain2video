...
def download_images_nod(target_path="./data", csail_username="cfosco"):
    path_to_images = (
        "/data/vision/oliva/scratch/datasets/NaturalObjectsDataset/stimuli/imagenet/*"
    )
...
def download_betas_cifti_nod(
    ...
    for subject in subjects_to_download:
        ...
        full_target_path = (
            f"{target_path}/betas_cifti_nod/sub{subject:02d}/prepared_allvoxel_pkl"
        )
        os.makedirs(full_target_path, exist_ok=True)
...
if __name__ == "__main__":
    ...
    if args.data_type in ["videos_bmd", "all"]:
        download_videos_bmd(args.target_path, args.csail_username)
    if args.data_type in ["videos_had", "all"]:
    ...
    if args.data_type in ["betas_impulse_bmd", "all"]:
        download_betas_glmsingle_impulse_bmd(
            args.target_path, args.csail_username, args.subjects_to_download
        )
    if args.data_type in ["betas_cifti_bmd", "all"]:
        download_betas_cifti_bmd(
            args.target_path, args.csail_username, args.subjects_to_download
        )
    if args.data_type in ["betas_cifti_had", "all"]:
        download_betas_cifti_had(
            args.target_path, args.csail_username, args.subjects_to_download
        )
