...
class CustomStableVideoDiffusionPipeline(StableVideoDiffusionPipeline):
    @torch.no_grad()
    ...
    def generate_from_image_embeddings(
        ...
        if self.do_classifier_free_guidance:
            ...
            image_shape = (1, 3, height, width)
        # noise = randn_tensor(image_shape, generator=generator, device=device, dtype=image_embeddings.dtype)
        noise = randn_tensor(image_shape, generator=generator, device=device)
        image = noise_aug_strength * noise
    ...
    @torch.no_grad()
    def generate_from_image_embeddings_latents(
        self,
        image_embeddings,
        image_latents,
        height: int = 576,
        ...
        if self.do_classifier_free_guidance:
            ...
            # TODO: Need to add some noise to the latents?
        ...
        # image_latents [batch, channels, height, width] ->[batch, num_frames, channels, height, width]
        image_latents = image_latents.unsqueeze(1).repeat(1, num_frames, 1, 1, 1)
        print(f"repeated image_latents: {image_latents.shape}")
...
if __name__ == "__main__":
    pipe: CustomStableVideoDiffusionPipeline = (
    ...
    export_to_video(out, "tmp.mp4", fps=7)
